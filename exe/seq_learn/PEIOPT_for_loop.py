...
class Opt_func:
    ...
    def __init__(self, y, model, yinfdata, option):# コンストラクタ
        ...
        #最適化する目的変数を決める
        for obj in range(0,len(self.yinf)):
            if np.abs(self.yinf[obj][0]) > 0.5:
#                print("hello")
                self.optobj = obj
                break
...
class result_cont:
    ...
    結果の格納
    """ 
#num_of_files = len(os.listdir("./xevals"))
#for i in range(num_of_files):
#    x_path = "./x_csvs/" + os.listdir("./x_csvs")[i]
#    y_path = "./y_csvs/" + os.listdir("./y_csvs")[i]
#    xeval_path = "./xevals/" + os.listdir("./xevals")[i]
#    yeval_path = os.listdir("./yevals")[i]
#
#    """
#最適化手法本体
#    """ 
...
#    ycsv = pd.read_csv(y_path, header=None)
#    y = ycsv.as_matrix()
...
#            #評価を行うリストを読み込み
#            Xeval = np.loadtxt(xeval_path,delimiter=",")
