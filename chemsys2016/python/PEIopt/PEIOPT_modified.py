...
""" 
# データロード
X = np.loadtxt("x_50.csv",delimiter=",")
X = X.astype(np.float64)#設計変数、説明変数
#print("X= ", X)
ycsv = pd.read_csv("y_logS_50.csv", header=None)
y = ycsv.as_matrix()
y = y.astype(np.float64)#特性、目的変数
#print("y= ", y)
...
for i in range(0,int(option[1][1])):
    ...
    if int(option[1][4]) == 0:
        ...
        #評価を行うリストを読み込み
        Xeval = np.loadtxt("Xeval_20.csv",delimiter=",")
...
#結果の出力
result.to_csv("result_logS_50_4_cand_1.csv")